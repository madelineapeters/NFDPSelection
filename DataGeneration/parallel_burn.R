...
#args=c("SIM","dummy_linear",5,1,1,"dummy",1000,1)
...
simFUN = function(i,x,POPlist,kb,SIM,FREQlist,fGENlist,type,alpha,mf){
  ...
  finalFreq = 0
  while (finalFreq < 0.98){
    seed = sample(1:10000000, 1)
    
    #Make subfolders to manipulate .vcf files
    #for (POP in POPlist){
      #for (freq in FREQlist){
        #mkDIR(POP,x,i,freq,type)
      #}
      #for (gen in fGENlist){
        #mkDIR(POP,x,i,gen,type)
      #}
    #}
  ...
  tmp = try({
    ...
    #Run SLiM
    #runSLiM(c(seed,alpha,x,i,SIM,type,mf))
    ...
    for (POP in POPlist){
      ...
      for (gen in fGENlist){
        sliceVCF(POP,x,i,kb,gen,type)
        sum.stat.list = sum.stat.pos(x,i,POP,kb,gen,type)
        #rmDIR(POP,x,i,gen,type)
        sum.stat.tmp = sum.stat.list[1:15] %>% unlist
        pos.stat.tmp = as.data.frame(sum.stat.list[16:18])
        pos.stat.tmp$spec = gen
        pos.stat.tmp$alpha = alpha
        pos.stat.tmp$mf = mf
        pos.stat.tmp$ID = i
        sum.stat.joint = append(sum.stat.joint,sum.stat.tmp)
        pos.stat.joint = bind_rows(pos.stat.joint,pos.stat.tmp)
        print(paste("Done",gen,sep=" "))
      }
    }
...
detest = detest.pair[1:183]
detest2 = detest.pair[184:190] %>% as.data.frame

if (length(unlist(detest)) == (length(POPlist)*15*(length(FREQlist)+length(fGENlist))+3)){
  
  #Convert simFUN output to dataframe
  ...
  #sub.namelist = c("Freq","pi","SegSites","TajD","HapNum","Fixed","Singletons","Doubletons","H1","H12","Singletons.Fixed")
  nameslist = c("mf","s",apply(expand.grid(apply(expand.grid(sub.namelist,c("sel")), 1, paste, collapse="."),c(FREQlist,fGENlist)),1,paste, collapse="."),"seed")
  names(sum.stat.df) = nameslist
  
  #Append summary statistics for each population to their respective .csv files
  for (POP in POPlist){
    for (spec in c(FREQlist,fGENlist)){
      
      POPpath=paste0(type,"_",POP)
      
      #Check for .vcf files that weren't deleted and delete if found
      file.list = list.files(path = paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,sep=""), pattern = paste(POP,"_",x,"_",sim,"_",spec,sep=""))
      
      if (length(file.list) > 0){
        rmVCFs= function(POP,x,sim,spec){
          output = system2("rm",c("-r",paste0("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/",POP,"_",x,"_",sim,"_",spec),"2>","/dev/null"),stdout=T)
        }
        rmVCFs(POP,x,sim,spec)
      }
      
    } #end loop over generations
    
    #Write temporary .csv file, remove its header and then glue to master .csv file
    df = sum.stat.df
    df$ID = sim
    df2 = detest2
    if (sim > 1){
      
      #Write as temporary .csv
      write.csv(df,paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/sumstats_",x,"_",POP,"tmp.csv",sep=""),row.names=FALSE)
      
      write.csv(df2,paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"tmp.csv",sep=""),row.names=FALSE)
      ...
      system2("awk",c("'FNR > 1'",paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"tmp.csv",sep=""),">",paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"noheading.csv",sep="")),stdout=T)
      ...
      system2("cat",c(paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"noheading.csv",sep=""),">>",paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/",SIM,"_",kb,"_pos_",x,"_",POP,"Freq.csv",sep="")),stdout=T)
      
      #Remove temporary file
      system2("rm",c(paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/sumstats_",x,"_",POP,"tmp.csv",sep="")),stdout=T)
      
      system2("rm",c(paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"tmp.csv",sep="")),stdout=T)
      
      #Remove file without header
      system2("rm",c(paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/sumstats_",x,"_",POP,"noheading.csv",sep="")),stdout=T)
      
      system2("rm",c(paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/pos_",x,"_",POP,"noheading.csv",sep="")),stdout=T)
      ...
      write.csv(df2,paste("/scratch/peter114/PopDataProcessing/SIM_VCFs/",POPpath,"/",SIM,"_",kb,"_pos_",x,"_",POP,"Freq.csv",sep=""),row.names=FALSE)
      
    }
  } #end of loop over populations
a #end if statement RE length of detest
